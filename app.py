...
app.secret_key = os.environ.get(
    "FLASK_SECRET_KEY", "TROQUE-PARA-UMA-CHAVE-FORTE-123456"
)
CORS(app)
...
# No início do arquivo, depois dos imports, adicione:
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Depois, altere o DB_PATH:
...
PIXEL_GIF = base64.b64decode(
    "R0lGODlhAQABAPAAAP///////yH5BAAAAAAALAAAAAABAAEAAAICRAEAOw=="
)
...
def init_tracking_db():
    ...
    cur = conn.cursor()
    cur.execute(
        "CREATE TABLE IF NOT EXISTS email_sends (id INTEGER PRIMARY KEY AUTOINCREMENT, operator TEXT NOT NULL, campaign TEXT NOT NULL, client_id TEXT, email TEXT, sent_at TEXT NOT NULL, ip TEXT, user_agent TEXT)"
    )
    cur.execute(
        "CREATE TABLE IF NOT EXISTS email_opens (id INTEGER PRIMARY KEY AUTOINCREMENT, token TEXT UNIQUE NOT NULL, operator TEXT NOT NULL, campaign TEXT NOT NULL, client_id TEXT, email TEXT, opened_at TEXT NOT NULL, ip TEXT, user_agent TEXT, device TEXT, referer TEXT)"
    )
    cur.execute(
        "CREATE TABLE IF NOT EXISTS link_clicks (id INTEGER PRIMARY KEY AUTOINCREMENT, token TEXT UNIQUE NOT NULL, operator TEXT NOT NULL, campaign TEXT NOT NULL, client_id TEXT, url TEXT NOT NULL, clicked_at TEXT NOT NULL, ip TEXT, user_agent TEXT, device TEXT, referer TEXT)"
    )
    cur.execute(
        "CREATE TABLE IF NOT EXISTS operator_access (id INTEGER PRIMARY KEY AUTOINCREMENT, operator TEXT NOT NULL, accessed_at TEXT NOT NULL, ip TEXT, user_agent TEXT, device TEXT)"
    )
    cur.execute(
        "CREATE TABLE IF NOT EXISTS short_links (id INTEGER PRIMARY KEY AUTOINCREMENT, short_code TEXT UNIQUE, original_url TEXT, created_at TEXT)"
    )
    conn.commit()
    conn.close()
...
def is_valid_operator_slug(slug: str) -> bool:
    return (
        bool(re.fullmatch(r"[a-z0-9_-]{2,32}", slug or ""))
...
def verify_token(token: str) -> dict | None:
    try:
        ...
        if not hmac.compare_digest(
            hmac.new(TRACKING_SECRET, payload, hashlib.sha256).hexdigest(), sig
        ):
            return None
...
def make_tracking_token(
    operator: str, campaign: str, client_id: str, email: str = "", ts: int = None
) -> str:
    if ts is None:
        ts = int(time.time())
    return sign_payload(
...
def record_operator_access(operator: str):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO operator_access (operator, accessed_at, ip, user_agent, device) VALUES (?, ?, ?, ?, ?)",
        (
            operator,
            now_utc(),
            client_ip(),
            request.headers.get("User-Agent", ""),
            device_from_ua(request.headers.get("User-Agent", "")),
        ),
    )
    conn.commit()
    conn.close()
...
def record_email_open(token: str, payload: dict):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT OR IGNORE INTO email_opens (token, operator, campaign, client_id, email, opened_at, ip, user_agent, device, referer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (
            token,
...
def record_link_click(token: str, payload: dict, url: str):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT OR IGNORE INTO link_clicks (token, operator, campaign, client_id, url, clicked_at, ip, user_agent, device, referer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (
            token,
            payload.get("operator", ""),
            payload.get("campaign", ""),
...
def tracking_dashboard():
    ...
    return render_template(
        ...
        latest_interactions=stats["latest"],
        agora=agora(),
    )
    ...
    if not operator or not campaign or not client_id:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "operator, campaign e client_id são obrigatórios",
                }
            ),
            400,
        )
...
def clear_tracking():
    ...
    cur = conn.cursor()
    cur.execute(
        "DELETE FROM email_opens WHERE operator = ? AND date(opened_at) = ?",
        (operador, hoje),
    )
    cur.execute(
        "DELETE FROM link_clicks WHERE operator = ? AND date(clicked_at) = ?",
        (operador, hoje),
    )
    cur.execute(
        "DELETE FROM operator_access WHERE operator = ? AND date(accessed_at) = ?",
        (operador, hoje),
    )
    conn.commit()
    conn.close()
...
def daily_stats():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT COUNT(*) as total FROM email_sends WHERE operator = ? AND date(sent_at) = ?",
        (operador, hoje),
    )
    sent = cur.fetchone()["total"]
    cur.execute(
        "SELECT COUNT(*) as total FROM email_opens WHERE operator = ? AND date(opened_at) = ?",
        (operador, hoje),
    )
    opens = cur.fetchone()["total"]
    cur.execute(
        "SELECT COUNT(*) as total FROM link_clicks WHERE operator = ? AND date(clicked_at) = ?",
        (operador, hoje),
    )
