...
def registrar_tentativa():
    ...
    try:
        ...
        if not tracking_id:
            ...
            print(f"📱 Criado novo tracking_id: {tracking_id} - Dispositivo: {device['resumo']}")
        ...
        if dados.get("usuario") and tracking_id:
            ...
            print(f"👤 Dados do usuário atualizados: {usuario.get('nome')} - CPF: {cpf_limpo}")
        
        conn.commit()
        conn.close()
        
        print(f"✅ Transferência {transferencia_id} registrada com sucesso em {agora_brasilia}")
    ...
    try:
        ...
        for row in rows:
            transferencias.append({
                ...
                "nome": row["nome"],
                "email": row["email"],
                "dispositivo": f"{row['dispositivo_tipo']} - {row['dispositivo_os']}" if row['dispositivo_tipo'] else "N/A",
...
def detalhe_transferencia(transferencia_id):
    ...
    try:
        ...
        if not row:
            return jsonify({"success": False, "error": "Transferência não encontrada"}), 404
        
        return jsonify({
            "success": True,
            "transferencia": dict(row)
        })
